...
export default {
  ...
  methods: {
    ...
    getLiveImages: async function () {
      for (const coral of this.listaNiveles) {
        let ruta = coral.live_coral;
        ruta = ruta.replace("coralImages", "images");
        let id = coral.corals_id;

        await axios
          .get("http://localhost:8000/Arrecife" + ruta) //TODO
          .then((answer) => {
            this.dictLiveImages[id] = answer.data; //TODO
          })
          .catch((error) => {
            this.dictLiveImages[id] = "";
          });
      }
